...
Output: duplicate eleent of the array
...
Data structure / Algorithms
    create an array of n elements
        iterate through the given array
            if the current element is in the new array, return the current element
            if not, store the current the element in the new array


'''

def find_dup(list):
    unique_list = [None] * len(list)

    for index, ele in enumerate(list):
        if ele in unique_list:
            return ele
        unique_list[index] = ele



print(find_dup([1, 5, 3, 1]) == 1)
print(find_dup([18,  9, 36, 96, 31, 19, 54, 75, 42, 15,
